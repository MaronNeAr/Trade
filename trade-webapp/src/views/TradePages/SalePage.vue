...
<div>
    ...
    <nut-row v-if="qcMenuShow && bcMenuShow">
        <nut-col :span="13">
            ...
            <br />
            <el-input-number class="line-style" size="large" v-model="state.price" :step="Math.max(state.maxPrice / 100, 0.000001)" :precision="6" :min="0" :max="state.maxPrice"></el-input-number>
            <br />
            <br />
    ...
    setup() {
        ...
        const methods = {
            ...
            sell: async () => {
                ...
                const response = await HttpManager.sell(params) as ResponseBody;
        ...
        const getTickers = async () => {
            await axios.get(baseURL + '/market/detail/merged?symbol=' + state.scType).then(response => {
                ...
                state.maxPrice = tick.ask[0];
                state.number = tick.ask[1];
                state.minNumber = tick.ask[1];
                state.status = true;
                let params = new URLSearchParams();
                params.append("content", JSON.stringify(response.data.tick));
